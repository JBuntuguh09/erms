...
const entity_type = require("../db/models/entity_type");
...
entity_type.belongsTo(user, {foreignKey: 'createdBy', as: 'user'});
entity_type.belongsTo(community, {foreignKey: 'community_id', as: 'community'});
entity_type.belongsTo(district, {foreignKey: 'district_id', as: 'district'});
entity_type.belongsTo(region, {foreignKey: 'region_id', as: 'region'});
...
const searchAll = catchAsync(async (req, res, next) => {
    ...
    if ( typeof query !== "string") {
        ...
        const result = await entity_type.findAll({
        ...
        if (!result || result.length === 0) {
            ...
            return next(new AppError("No matching entity types found", 400));
        }

        return res.status(200).json({
            status: "Success",
            message: "Retrieved entity types successfully",
            data: result
        });
    } catch (error) {
...
const insertData=catchAsync(async(req, res, next)=>{
    try {
        ...
        const nData = await entity_type.create(occData);
        if (!nData) {
            return next(new AppError("Failed to create new entity type", 400));
        ...
        return res.status(201).json({
          status: "Success",
          message: "New entity type successfully created",
          data: nData,
        });
      } catch (error) {
