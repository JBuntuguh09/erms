...
property_type.belongsTo(user, {foreignKey: 'createdBy', as: 'user'});
property_type.belongsTo(community, {foreignKey: 'community_id', as: 'community'});
property_type.belongsTo(district, {foreignKey: 'district_id', as: 'district'});
property_type.belongsTo(region, {foreignKey: 'region_id', as: 'region'});
...
const insertData=catchAsync(async(req, res, next)=>{
    try {
        ...
        const nData = await property_type.create(occData);
        if (!nData) {
